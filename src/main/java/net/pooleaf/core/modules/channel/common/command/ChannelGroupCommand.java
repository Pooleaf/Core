...
public class ChannelGroupCommand {
  ...
  public static void channelGroupList(CommonCommandSender sender, CommandResult result) {
    sender.nmessage("§e[ 채널 그룹 목록 ]");
    for (ChannelGroup channelGroup : ChannelModule.getChannelGroups()) {
      sender.nmessage(channelGroup.getName() + "§e(" + channelGroup.getDisplayName() + ")");
    }
  }
  ...
  public static void channelGroupInfo(CommonCommandSender sender, CommandResult result) {
  ...
  @Command(
      ...
      arguments = "<채널그룹> <표기>",
      description = "채널 그룹의 표기를 설정합니다.",
      permission = ChannelPermission.ADMIN
  )
  public static void channelGroupSetDisplayName(CommonCommandSender sender, CommandResult result) {
    ChannelGroup channelGroup = ChannelModule.getChannelGroup(result.getEnteredArguments());
    if (channelGroup == null) {
      sender.warning("존재하지 않는 채널 그룹입니다.");
      return;
    ...
    sender.message(channelGroup.getName() + " §e채널의 표기를 §f" + displayName + "§e(으)로 설정했습니다.");
  }
  ...
  public static void channelGroupJoin(CommonCommandSender sender, CommandResult result) {
    ...
    if (target.equals("현재") || target.equalsIgnoreCase("current")) {
      ...
      sender.message(joinRequestCount + "§e명의 플레이어 중 §f" + joinSuccessCount + "§e명의 플레이어를 §f" + channelGroup.getName() + " §e그룹의 채널로 빠른접속 시키는데 성공했습니다.");
    }
    // 모든 플레이어 이동시키기
  ...
  public static void channelBroadcast(CommonCommandSender sender, CommandResult result) {
    ...
    sender.message(channelGroup.getName() + " §e채널 그룹에 §f'" + message + "' §e메시지를 공지했습니다.");
  }
  ...
  public static void channelRemoteCommand(CommonCommandSender sender, CommandResult result) {
    ...
    sender.message(channelGroup.getName() + " §e채널 그룹에 §f'" + commandLine + "' §e명령어를 보냈습니다.");
  }
